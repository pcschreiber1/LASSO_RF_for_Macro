...
ret_lasso = ret_lasso2 = ret_RF = nonzero_lasso = nonzero_lasso2 = nonzero_RF = pred_lasso = pred_lasso2 = pred_RF = matrix(NaN, ncol=length(snr.vec), nrow=n_sim) # each column corresponds to SNR value

vec_colnames = c()
for (j in list("ret", "zero", "pred")){
  for (i in list("LASSO", "LASSO2", "RF")){
    vec_colnames <- c(vec_colnames, paste(j, i,round(snr.vec, digits=2)), sep="_")
  }
}
...
for (j in 1:length(snr.vec)){
  ...
  for (i in 1:n_sim){
    ...
    ret_lasso[i, j] = res_lasso$retention   
    nonzero_lasso[i, j] = res_lasso$nonzero
    pred_lasso[i, j] = res_lasso$mse
    
    res_lasso2 = cv.relaxed_lasso(data=df, beta=beta)
    ret_lasso2[i, j] = res_lasso2$retention   
    nonzero_lasso2[i, j] = res_lasso2$nonzero
    pred_lasso2[i, j] = res_lasso2$mse
    
    res_RF = RF_VSURF(data=df, beta=beta)
    ret_RF[i, j] = res_RF$retention   
    nonzero_RF[i, j] = res_RF$nonzero
    pred_RF[i, j] = res_RF$OOB_error
    
    
    #Save results
    sim1= data.frame(cbind(
      ret_lasso, ret_lasso2, ret_RF, 
      nonzero_lasso,nonzero_lasso,nonzero_RF,
      pred_lasso, pred_lasso2,pred_RF))
    #colnames(sim1) = vec_colnames
    write.csv(sim1,"sim1.csv", row.names = FALSE)
  }
}
